...
const HodDashboard = () => {
  ...
  const [privateEvents, setPrivateEvents] = useState(() => {
    const saved = localStorage.getItem(`private_calendar_events_hod_${user?.id}`);
  ...
  useEffect(() => {
    if (user?.id) {
      localStorage.setItem(`private_calendar_events_hod_${user.id}`, JSON.stringify(privateEvents));
    }
  }, [privateEvents, user]);
  ...
  const [tasks, setTasks] = useState(() => {
    const saved = localStorage.getItem(`hod_tasks_${user?.id}`);
    if (saved) {
      try {
    ...
    return [
      { 
        id: 1, 
        title: 'Review and approve Semester Results draft', 
        subject: 'Administration', 
        dueDate: '2026-07-22', 
        status: 'inprogress',
        comments: [
          { id: 101, author: 'Dr. Connor', text: 'Waiting for student-service results replication to finish.', date: 'Jul 10, 11:30 AM' }
        ]
      },
      { 
        id: 2, 
        title: 'Set DBMS CAT2 Exam schedule', 
        subject: 'Academic Scheduling', 
        dueDate: '2026-07-18', 
        status: 'todo',
        comments: [] 
      }
  ...
  useEffect(() => {
    if (user?.id) {
      localStorage.setItem(`hod_tasks_${user.id}`, JSON.stringify(tasks));
    }
  }, [tasks, user]);
  ...
  const handleAddComment = () => {
    ...
    const newComment = {
      id: Date.now(),
      author: 'Dr. ' + (user?.name ? user.name.split(' ').pop() : 'Connor'),
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="dashboard-split-layout">
        ...
        {/* Right Column: Operational Scope Settings & Calendar */}
